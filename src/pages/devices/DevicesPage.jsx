...
export default function DevicesPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    getDeviceInventory({ search, page, size: PAGE_SIZE, sort, direction, projectId, groupId, tagIds })
    ...
    return () => { active = false; };
  }, [search, page, sort, direction, projectId, groupId, tagIds, reloadKey]);
  ...
  return (
    <section className="device-inventory-page">
      ...
      <form className="device-inventory-controls" onSubmit={submitSearch}>
        ...
        {(groupId || tagIds.length > 0) && <button type="button" onClick={() => { setGroupId(""); setTagIds([]); setPage(0); }}>Clear filters</button>}
        ...
        <>
          ...
          <div className="device-inventory-cards">{devices.map(device => (
            <article key={device.deviceId} className={`device-inventory-card ${device.operationalStatus?.toLowerCase()}`}>
              ...
              <dl><div><dt>Type</dt><dd>{device.type}</dd></div><div><dt>Lifecycle</dt><dd>{device.active ? "Active" : "Inactive"}</dd></div><div><dt>Heartbeat</dt><dd>{device.heartbeatStatus.replaceAll("_", " ")} · {formatDate(device.latestHeartbeat)}</dd></div><div><dt>Firmware</dt><dd>{optionalValue(device.firmwareVersion)}</dd></div><div><dt>Project</dt><dd>{optionalValue(device.assignedProject)}</dd></div><div><dt>Registered</dt><dd>{formatDate(device.registrationDate)}</dd></div><div><dt>Last communication</dt><dd>{formatDate(device.lastSeen)}</dd></div><div><dt>Groups</dt><dd>{device.groups?.length ? device.groups.map(group => group.name).join(", ") : "None"}</dd></div><div><dt>Tags</dt><dd>{device.tags?.length ? device.tags.join(", ") : "None"}</dd></div><div><dt>Location</dt><dd>{optionalValue(device.location)}</dd></div></dl>
